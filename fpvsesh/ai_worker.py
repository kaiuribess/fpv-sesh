...
def render(config):
    ...
    pixel = "p010le" if codec == "hevc" else "yuv420p"
    ...
    encode = [str(ffmpeg), "-hide_banner", "-loglevel", "error", "-y", "-f", "rawvideo",
              ...
              "-i", "pipe:0", "-an", "-vf", ",".join(vf), "-frames:v", str(frames),
              "-c:v", "hevc_nvenc" if codec == "hevc" else "h264_nvenc",
              "-preset", config.get("encoder_preset", "p7"), "-tune", "hq", "-rc", "vbr",
              "-cq", str(config.get("cq", 16)), "-b:v", "0", "-spatial-aq", "1",
              "-temporal-aq", "1", "-rc-lookahead", "32", "-multipass", "fullres"]
    if codec == "hevc":
        encode += ["-profile:v", "main10", "-tag:v", "hvc1", "-tier", "high", "-maxrate", "120M"]
    ...
    result = {"ai_inference": True, "backend": "real-esrgan-pytorch-cuda",
              ...
              "cq": config.get("cq", 16), "encoder_preset": config.get("encoder_preset", "p7")}
